 ...
 ** 4DOS OPTION command
 **
 ** Copyright 1997-2002 JP Software Inc., All rights reserved
 *************************************************************************
 ** Compiler #defines:
...
static int _ExecuteDialogs(int nCategory) {
	...
	nHoldLen = gpINIPtr->InstallPath != INI_EMPTYSTR ? strlen(pszPath) : 0;
		...
		*pfUpdate = TRUE;
		...
		case INI_CTL_TEXT:
			...
			if (DlgGetFieldString(uIDIIndex, szBuffer, 511) != 0) {
				// Error
#ifdef DEBUG
				printf("  Could not get text value for %u  ", uIDIIndex);
				getchar();
#endif  // DEBUG
			}
			else {
				...
				if ((INI_PTMASK & gaINIItemList[uINIItemNum].cParseType) ==
					...
					if (fStringChanged) {
						ini_string(gpINIPtr, (unsigned int *)pDataPtr, szBuffer,
						           strlen(szBuffer));
						*pfUpdate = TRUE;
					}
